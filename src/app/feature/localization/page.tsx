"use client";

/* ─── Design token helper ─── */
const V = (n: string) => `var(--${n})`;

/* ─── Styles ─── */
const S = {
  h1: { fontSize: "80px", fontWeight: 600, lineHeight: "83.2px", letterSpacing: "-0.8px" } as React.CSSProperties,
  h2: { fontSize: "56px", fontWeight: 600, lineHeight: "58.24px" } as React.CSSProperties,
  h3: { fontSize: "32px", fontWeight: 500, lineHeight: "41.6px" } as React.CSSProperties,
  h4: { fontSize: "20px", fontWeight: 600, lineHeight: "28px" } as React.CSSProperties,
  body: { fontSize: "16px", fontWeight: 400, lineHeight: "25.6px" } as React.CSSProperties,
  bodySm: { fontSize: "14px", lineHeight: "22.4px" } as React.CSSProperties,
  caption: { fontSize: "12.8px", fontWeight: 550, lineHeight: "15.36px" } as React.CSSProperties,
  eyebrow: { fontSize: "15px", fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase" as const } as React.CSSProperties,
  btn: { fontSize: "16px", fontWeight: 500, lineHeight: "19.2px", padding: "16px 24px", borderRadius: "4px" } as React.CSSProperties,
};

/* ─── Shared components ─── */
function Arrow() {
  return <span className="ml-1 text-[1.1em] leading-none select-none">&rarr;</span>;
}

/* ─── Wordmark ─── */
function Wordmark({ className = "" }: { className?: string }) {
  return (
    <span className={`font-heading font-[800] text-xl tracking-tight ${className}`} style={{ color: V("color-ink") }}>
      Kreature<span style={{ color: V("color-accent-blue") }}>.</span>
    </span>
  );
}

/* ─── LOCALIZATION FEATURES ─── */
const LOCALIZATION_FEATURES = [
  {
    icon: "🌐",
    title: "Automatic translation",
    desc: "AI-powered machine translation that instantly converts your content into dozens of languages. Powered by state-of-the-art neural models that understand context, idioms, and brand voice.",
  },
  {
    icon: "✍️",
    title: "Manual translation control",
    desc: "Fine-tune every translation with a side-by-side editor. Invite translators, leave comments, and approve changes before they go live. Full version history on every string.",
  },
  {
    icon: "📍",
    title: "Locale-specific content",
    desc: "Not just translation — true localization. Customize images, CTAs, pricing, and promotions per locale. Show different testimonials and case studies in each market.",
  },
  {
    icon: "🔍",
    title: "SEO for every locale",
    desc: "Auto-generated hreflang tags, localized meta titles and descriptions, translated URLs, and per-locale sitemaps. Every page is crawlable and indexable in every language you support.",
  },
  {
    icon: "↔️",
    title: "RTL support",
    desc: "Full right-to-left layout support for Arabic, Hebrew, Farsi, and Urdu. Text, navigation, forms, and components all mirror correctly with zero custom CSS.",
  },
  {
    icon: "💱",
    title: "Currency formatting",
    desc: "Automatic currency conversion and locale-aware formatting. Display prices in local currency with proper symbols, decimal separators, and thousands grouping per region.",
  },
];

/* ─── SUPPORTED LANGUAGES ─── */
const SUPPORTED_LANGUAGES = [
  { code: "EN", name: "English", flag: "🇬🇧" },
  { code: "ES", name: "Spanish", flag: "🇪🇸" },
  { code: "FR", name: "French", flag: "🇫🇷" },
  { code: "DE", name: "German", flag: "🇩🇪" },
  { code: "IT", name: "Italian", flag: "🇮🇹" },
  { code: "PT", name: "Portuguese", flag: "🇵🇹" },
  { code: "NL", name: "Dutch", flag: "🇳🇱" },
  { code: "PL", name: "Polish", flag: "🇵🇱" },
  { code: "RU", name: "Russian", flag: "🇷🇺" },
  { code: "ZH", name: "Chinese (Simplified)", flag: "🇨🇳" },
  { code: "ZH", name: "Chinese (Traditional)", flag: "🇹🇼" },
  { code: "JA", name: "Japanese", flag: "🇯🇵" },
  { code: "KO", name: "Korean", flag: "🇰🇷" },
  { code: "AR", name: "Arabic", flag: "🇦🇪" },
  { code: "HE", name: "Hebrew", flag: "🇮🇱" },
  { code: "HI", name: "Hindi", flag: "🇮🇳" },
  { code: "TR", name: "Turkish", flag: "🇹🇷" },
  { code: "TH", name: "Thai", flag: "🇹🇭" },
  { code: "VI", name: "Vietnamese", flag: "🇻🇳" },
  { code: "ID", name: "Indonesian", flag: "🇮🇩" },
  { code: "SV", name: "Swedish", flag: "🇸🇪" },
  { code: "DA", name: "Danish", flag: "🇩🇰" },
  { code: "FI", name: "Finnish", flag: "🇫🇮" },
  { code: "NO", name: "Norwegian", flag: "🇳🇴" },
  { code: "CS", name: "Czech", flag: "🇨🇿" },
  { code: "RO", name: "Romanian", flag: "🇷🇴" },
  { code: "UK", name: "Ukrainian", flag: "🇺🇦" },
  { code: "EL", name: "Greek", flag: "🇬🇷" },
  { code: "HU", name: "Hungarian", flag: "🇭🇺" },
  { code: "MS", name: "Malay", flag: "🇲🇾" },
  { code: "TL", name: "Filipino", flag: "🇵🇭" },
  { code: "BN", name: "Bengali", flag: "🇧🇩" },
];

/* ─── HOW IT WORKS ─── */
const HOW_IT_WORKS = [
  {
    step: "01",
    title: "Setup your locales",
    desc: "Choose the languages and regions you want to target. Configure your default locale, fallback rules, and enable automatic language detection based on visitor browser preferences.",
  },
  {
    step: "02",
    title: "Translate your content",
    desc: "Use AI-powered automatic translation for speed, or hand off to human translators for quality. Our side-by-side editor shows source and translation together — review, edit, and approve with full context.",
  },
  {
    step: "03",
    title: "Publish localized pages",
    desc: "Hit publish and your localized content goes live instantly. Each locale gets its own URL structure, hreflang tags, and sitemap entry — fully optimized for search engines in every market.",
  },
];

/* ─── ENTERPRISE CAPABILITIES ─── */
const ENTERPRISE_CAPABILITIES = [
  {
    title: "Translation memory",
    desc: "Reuse previously translated strings across projects. Our TM stores every approved translation and suggests matches when similar content appears — reducing cost by up to 60%.",
    highlight: "Save up to 60%",
  },
  {
    title: "Glossary management",
    desc: "Define brand-specific terminology, product names, and style rules per language. The translation engine respects your glossary, ensuring consistent brand voice in every market.",
    highlight: "Brand consistency",
  },
  {
    title: "Workflow automation",
    desc: "Set up approval chains and role-based permissions. Automatically route content to translators, reviewers, and publishers. Integrate with your existing CI/CD pipeline for automated deployments.",
    highlight: "Enterprise workflows",
  },
  {
    title: "Analytics per locale",
    desc: "Track traffic, engagement, and conversions broken down by language and region. See which markets are growing, where content gaps exist, and where to invest translation efforts next.",
    highlight: "Locale-level insights",
  },
  {
    title: "API-first architecture",
    desc: "REST and GraphQL endpoints expose every translation for programmatic access. Pull translations into mobile apps, email campaigns, and third-party platforms automatically.",
    highlight: "Headless by design",
  },
  {
    title: "Compliance ready",
    desc: "GDPR-compliant data handling with data residency options. SOC 2 Type II certified infrastructure. Audit logs track every translation change across your organization.",
    highlight: "SOC 2 + GDPR",
  },
];

/* ─── FOOTER DATA ─── */
const FOOTER_COLS = [
  {
    heading: "Product",
    links: [
      { label: "CMS", href: "/feature/cms" },
      { label: "Hosting", href: "/feature/hosting" },
      { label: "Design", href: "/feature/design" },
      { label: "AI", href: "/feature/ai" },
      { label: "SEO", href: "/feature/seo" },
      { label: "Localization", href: "/feature/localization" },
      { label: "Optimize", href: "/feature/optimize" },
    ],
  },
  {
    heading: "Solutions",
    links: [
      { label: "Enterprise", href: "/enterprise" },
      { label: "Agencies", href: "/solutions" },
      { label: "Ecommerce", href: "/feature/ecommerce" },
      { label: "Startups", href: "/solutions" },
    ],
  },
  {
    heading: "Resources",
    links: [
      { label: "Blog", href: "/blog" },
      { label: "Templates", href: "/templates" },
      { label: "Libraries", href: "/libraries" },
      { label: "Glossary", href: "/glossary" },
      { label: "Made in Webflow", href: "/made-in-webflow" },
    ],
  },
  {
    heading: "Company",
    links: [
      { label: "About", href: "/company" },
      { label: "Careers", href: "/company" },
      { label: "Customers", href: "/customers" },
      { label: "Partners", href: "/certified-partners" },
      { label: "Contact", href: "/contact-sales" },
    ],
  },
  {
    heading: "Compare",
    links: [
      { label: "vs WordPress", href: "/webflow-vs-wordpress" },
      { label: "vs Framer", href: "/webflow-vs-framer" },
      { label: "vs Squarespace", href: "/webflow-vs-squarespace" },
      { label: "vs Wix", href: "/webflow-vs-wix" },
    ],
  },
  {
    heading: "Community",
    links: [
      { label: "Forum", href: "/community" },
      { label: "Events", href: "/community" },
      { label: "Webflow TV", href: "/community" },
      { label: "Discord", href: "/community" },
    ],
  },
  {
    heading: "Get Help",
    links: [
      { label: "Support", href: "/contact-sales" },
      { label: "Docs", href: "/resources" },
      { label: "Status", href: "/resources" },
      { label: "Login", href: "/login" },
    ],
  },
];

/* ══════════════════════════════════════════════════════════════════════════════════════════
   LOCALIZATION FEATURE PAGE
   ══════════════════════════════════════════════════════════════════════════════════════════ */
export default function LocalizationPage() {
  return (
    <>
      <main>
        {/* ═══ HERO ═══ */}
        <section
          className="pt-[148px] pb-20 sm:pb-28 px-5 sm:px-8"
          style={{ background: V("color-canvas") }}
        >
          <div className="max-w-3xl mx-auto text-center">
            <p style={{ ...S.eyebrow, color: V("color-accent-green") }}>Localization</p>
            <h1 className="mt-4 mb-6" style={S.h1}>
              Go global, stay local
            </h1>
            <p className="max-w-2xl mx-auto" style={{ ...S.body, color: V("color-body-mid"), fontSize: "20px" }}>
              Launch multi-language sites that feel native in every market. Automatic translation,
              locale-specific content, and SEO built for global reach — all managed from one dashboard.
            </p>
            <div className="flex items-center justify-center gap-4 mt-10">
              <a
                href="/signup"
                className="no-underline inline-flex items-center"
                style={{
                  ...S.btn,
                  background: V("color-accent-blue"),
                  color: "#fff",
                }}
                onMouseEnter={(e) => (e.currentTarget.style.background = V("color-accent-blue-hover"))}
                onMouseLeave={(e) => (e.currentTarget.style.background = V("color-accent-blue"))}
              >
                Start localizing <Arrow />
              </a>
              <a
                href="#features"
                className="no-underline inline-flex items-center"
                style={{
                  ...S.btn,
                  background: "transparent",
                  color: V("color-ink"),
                  border: `1px solid ${V("color-hairline")}`,
                }}
              >
                Explore features
              </a>
            </div>
          </div>
        </section>

        {/* ═══ LOCALIZATION FEATURES ═══ */}
        <section
          id="features"
          className="py-20 sm:py-28 px-5 sm:px-8"
          style={{ background: V("color-canvas-soft") }}
        >
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <p style={{ ...S.eyebrow, color: V("color-accent-green") }}>Features</p>
              <h2 className="mt-3 mb-4" style={S.h2}>
                Everything you need to go multilingual
              </h2>
              <p className="max-w-xl mx-auto" style={{ ...S.body, color: V("color-body-mid") }}>
                From automatic translation to RTL layout support, our localization engine handles the
                complexity so you can focus on growing globally.
              </p>
            </div>

            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {LOCALIZATION_FEATURES.map((feat) => (
                <div
                  key={feat.title}
                  className="rounded-xl p-8 transition-shadow"
                  style={{
                    background: V("color-canvas"),
                    border: `1px solid ${V("color-hairline")}`,
                    boxShadow: V("shadow-card"),
                  }}
                  onMouseEnter={(e) => (e.currentTarget.style.boxShadow = V("shadow-card-strong"))}
                  onMouseLeave={(e) => (e.currentTarget.style.boxShadow = V("shadow-card"))}
                >
                  <div
                    className="w-12 h-12 rounded-lg flex items-center justify-center text-xl mb-5"
                    style={{ background: V("color-canvas-soft"), color: V("color-accent-green") }}
                  >
                    {feat.icon}
                  </div>
                  <h3 className="mb-3" style={{ ...S.h4, color: V("color-ink") }}>{feat.title}</h3>
                  <p style={{ ...S.bodySm, color: V("color-body-mid") }}>{feat.desc}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ═══ SUPPORTED LANGUAGES ═══ */}
        <section className="py-20 sm:py-28 px-5 sm:px-8" style={{ background: V("color-canvas") }}>
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <p style={{ ...S.eyebrow, color: V("color-accent-green") }}>Supported Languages</p>
              <h2 className="mt-3 mb-4" style={S.h2}>
                30+ languages and counting
              </h2>
              <p className="max-w-xl mx-auto" style={{ ...S.body, color: V("color-body-mid") }}>
                Every major language supported out of the box. We add new languages regularly
                based on customer demand.
              </p>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {SUPPORTED_LANGUAGES.map((lang) => (
                <div
                  key={lang.name}
                  className="rounded-xl p-5 text-center transition-shadow"
                  style={{
                    background: V("color-canvas-soft"),
                    border: `1px solid ${V("color-hairline")}`,
                  }}
                  onMouseEnter={(e) => (e.currentTarget.style.boxShadow = V("shadow-card-strong"))}
                  onMouseLeave={(e) => (e.currentTarget.style.boxShadow = "none")}
                >
                  <span className="text-3xl block mb-2">{lang.flag}</span>
                  <span
                    className="inline-block px-2 py-0.5 rounded text-xs font-semibold mb-1"
                    style={{ background: V("color-canvas"), color: V("color-accent-green") }}
                  >
                    {lang.code}
                  </span>
                  <p className="m-0 text-sm" style={{ color: V("color-body-mid") }}>{lang.name}</p>
                </div>
              ))}
            </div>

            <p className="text-center mt-8" style={{ ...S.bodySm, color: V("color-mute") }}>
              Don't see your language? <a href="/contact-sales" style={{ color: V("color-accent-blue"), textDecoration: "underline" }}>Contact us</a> — we add new languages monthly.
            </p>
          </div>
        </section>

        {/* ═══ HOW IT WORKS ═══ */}
        <section className="py-20 sm:py-28 px-5 sm:px-8" style={{ background: V("color-canvas-soft") }}>
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <p style={{ ...S.eyebrow, color: V("color-accent-green") }}>How it works</p>
              <h2 className="mt-3 mb-4" style={S.h2}>
                From one language to many in three steps
              </h2>
              <p className="max-w-xl mx-auto" style={{ ...S.body, color: V("color-body-mid") }}>
                Set up new locales, translate your content, and publish — all within your existing
                workflow. No separate tools or platforms required.
              </p>
            </div>

            <div className="grid sm:grid-cols-3 gap-8">
              {HOW_IT_WORKS.map((item) => (
                <div key={item.step} className="text-center sm:text-left">
                  <div
                    className="w-14 h-14 rounded-full flex items-center justify-center mb-5 mx-auto sm:mx-0"
                    style={{ background: V("color-accent-green"), color: "#fff" }}
                  >
                    <span style={{ ...S.h4, color: "#fff" }}>{item.step}</span>
                  </div>
                  <h3 className="mb-3" style={{ ...S.h4, color: V("color-ink") }}>{item.title}</h3>
                  <p style={{ ...S.bodySm, color: V("color-body-mid") }}>{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ═══ ENTERPRISE ═══ */}
        <section className="py-20 sm:py-28 px-5 sm:px-8" style={{ background: V("color-canvas") }}>
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <p style={{ ...S.eyebrow, color: V("color-accent-purple") }}>Enterprise</p>
              <h2 className="mt-3 mb-4" style={S.h2}>
                Advanced localization workflows
              </h2>
              <p className="max-w-xl mx-auto" style={{ ...S.body, color: V("color-body-mid") }}>
                Built for teams that manage content across dozens of markets. Translation memory,
                approval chains, and analytics that scale with your global operations.
              </p>
            </div>

            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {ENTERPRISE_CAPABILITIES.map((cap) => (
                <div
                  key={cap.title}
                  className="rounded-xl p-8"
                  style={{
                    background: V("color-canvas-soft"),
                    border: `1px solid ${V("color-hairline")}`,
                  }}
                >
                  <span
                    className="inline-block px-3 py-1 rounded-full mb-4"
                    style={{
                      ...S.caption,
                      background: V("color-canvas"),
                      color: V("color-accent-purple"),
                    }}
                  >
                    {cap.highlight}
                  </span>
                  <h3 className="mb-3" style={{ ...S.h4, color: V("color-ink") }}>{cap.title}</h3>
                  <p style={{ ...S.bodySm, color: V("color-body-mid") }}>{cap.desc}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ═══ CTA ═══ */}
        <section className="py-20 sm:py-28 px-5 sm:px-8" style={{ background: V("color-canvas-soft") }}>
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="mb-4" style={S.h2}>
              Start localizing
            </h2>
            <p className="max-w-xl mx-auto mb-10" style={{ ...S.body, color: V("color-body-mid"), fontSize: "20px" }}>
              Reach customers in their native language. Launch your first localized site today
              — no credit card required.
            </p>
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <a
                href="/signup"
                className="no-underline inline-flex items-center"
                style={{
                  ...S.btn,
                  background: V("color-accent-blue"),
                  color: "#fff",
                }}
                onMouseEnter={(e) => (e.currentTarget.style.background = V("color-accent-blue-hover"))}
                onMouseLeave={(e) => (e.currentTarget.style.background = V("color-accent-blue"))}
              >
                Get started free <Arrow />
              </a>
              <a
                href="/contact-sales"
                className="no-underline inline-flex items-center"
                style={{
                  ...S.btn,
                  background: "transparent",
                  color: V("color-ink"),
                  border: `1px solid ${V("color-hairline")}`,
                }}
              >
                Talk to sales
              </a>
            </div>
          </div>
        </section>
      </main>

      {/* ═══ FOOTER ═══ */}
      <footer style={{ background: V("color-canvas"), borderTop: `1px solid ${V("color-hairline")}` }}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-14 sm:py-16">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-8">
            {/* Brand column */}
            <div className="col-span-2 sm:col-span-3 lg:col-span-1 lg:row-span-1">
              <a href="/" className="flex items-center gap-2.5 mb-3 no-underline">
                <img src="/logo/kreature-logo-dark.png" alt="Kreature" className="logo-dark h-[36px] w-auto" />
                <img src="/logo/kreature-logo-light.png" alt="Kreature" className="logo-light h-[36px] w-auto" />
                <Wordmark />
              </a>
              <p className="text-sm leading-relaxed max-w-xs" style={{ color: V("color-mute") }}>
                AI Product Studio for founders who ship fast.
              </p>
            </div>

            {/* Link columns */}
            {FOOTER_COLS.map((col) => (
              <div key={col.heading} className="lg:justify-self-start">
                <div className="mb-4" style={{ ...S.caption, color: V("color-mute"), textTransform: "uppercase", letterSpacing: "0.1em" }}>
                  {col.heading}
                </div>
                <ul className="list-none p-0 m-0 space-y-3">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <a
                        href={link.href}
                        className="no-underline transition-colors"
                        style={{ ...S.bodySm, color: V("color-body-mid") }}
                        onMouseEnter={(e) => (e.currentTarget.style.color = V("color-ink"))}
                        onMouseLeave={(e) => (e.currentTarget.style.color = V("color-body-mid"))}
                      >
                        {link.label}
                      </a>
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </div>

          {/* Bottom bar */}
          <div
            className="mt-14 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4"
            style={{ borderTop: `1px solid ${V("color-hairline")}` }}
          >
            <p className="text-xs m-0" style={{ color: V("color-mute") }}>
              &copy; {new Date().getFullYear()} Kreature Studio. All rights reserved.
            </p>
            <div className="flex items-center gap-5">
              <a
                href="https://twitter.com"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Twitter"
                className="transition-colors"
                style={{ color: V("color-mute") }}
                onMouseEnter={(e) => (e.currentTarget.style.color = V("color-ink"))}
                onMouseLeave={(e) => (e.currentTarget.style.color = V("color-mute"))}
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href="https://linkedin.com"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="LinkedIn"
                className="transition-colors"
                style={{ color: V("color-mute") }}
                onMouseEnter={(e) => (e.currentTarget.style.color = V("color-ink"))}
                onMouseLeave={(e) => (e.currentTarget.style.color = V("color-mute"))}
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
            </div>
          </div>
        </div>
      </footer>
    </>
  );
}
